...
def admin_page():
    ...
    def adm_draw_index():
        for i in range(0,30):
            Label(admin_window,text=f"{i+1}.",width=3,height=1).grid(row=i+2,column=1)
    adm_draw_index()
    #Names
    def adm_draw_name():
        #입력창 그리기
    ...
    def adm_draw_count():
        for i in range(0,30):
            Label(admin_window,text=str(m.items_count[i])+'개',width=15,height=1).grid(row=i+2,column=7)
...
def draw_price():
    price_row=[2,4,6]
    for i in range(0,30):
        Label(root,text=str(m.items_price[i])+"원",width=15,height=1).grid(row=price_row[i//10],column=(i%10)+1)
def draw_func():
    def add_cash(num):
        if cash[num]:
            if num == 0:
